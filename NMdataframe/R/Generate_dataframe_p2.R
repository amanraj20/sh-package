...
generate_dataframe <- function(required_files, previous_function) {
  ...
  first_step <- predecessor %>% 
    dplyr::filter(source == main_file[1]) %>% 
    dplyr::select(VARIABLE, LABEL, variables)
  ...
  ultimate_file <- final_file %>% 
    dplyr::select(as.character(req_vars))
  ...
  if (length(required_files) > 1) {
    for (i in seq(2, length(required_files))) {
      ...
      use_file <- use_file %>% 
        dplyr::select(USUBJID, as.character(req_vars))

      ultimate_file <- ultimate_file %>% 
        dplyr::left_join(use_file, by = "USUBJID")
    }
  }
  ...
  ultimate_file <- ultimate_file %>% 
    dplyr::select(as.character(ordr))
