...
metadata <- function(path_metadata_file, create) {
  guide <- readxl::read_excel(path = path_metadata_file, sheet = "Variables") %>% 
    as.data.frame()
  
  to_create <- create
  
  guide1 <- guide %>% 
    dplyr::filter(DATASET == to_create)
  
  predecessor <- guide1 %>% 
    dplyr::filter(SOURCE_VAR != "",
                  is.na(SOURCE_VAR) == F,
                  lengths(strsplit(SOURCE_VAR, "\\W+")) < 3)

  assigned <- guide1 %>% 
    dplyr::filter(SOURCE_VAR == "" |
                  is.na(SOURCE_VAR) == T |
                  lengths(strsplit(SOURCE_VAR, "\\W+")) > 2)
  
  predecessor <- predecessor %>% 
    tidyr::separate(SOURCE_VAR, c("source", "variables"), sep = "\\.")
  
  # View(predecessor)
  
  to_import <- unique(c(predecessor$source))
  cat("Import these files from sdtm or adam folder: ", "\n", to_import)
  ...
  cat("After importing, add files as a list in this sequence in next function: generate_dataframe(required_files = list(), previous_function= __) ",
      "\n", main_file)
